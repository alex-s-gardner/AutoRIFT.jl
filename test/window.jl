using AutoRIFT: windowmax, windowmin, windowmean, windowmedian, windowmad,
                windowrange, count_agreeing, small_components
...
# Bit equality with `NaN == NaN`, for the comparisons against the reference's own output where a
# tolerance would be a claim rather than a measurement: these reductions select and average
# existing values, so agreement is exact or it is a semantic difference.
nan_same(a, b) = (isnan(a) && isnan(b)) || a == b
nan_identical(a, b) = size(a) == size(b) && all(i -> nan_same(a[i], b[i]), eachindex(a, b))
...
# ---------------------------------------------------------------------------
# Against the reference's own `colfilt`
# ---------------------------------------------------------------------------
#
# The sweep above checks each reduction against a brute-force statement of its documented
# semantics. That establishes internal consistency and says nothing about whether the semantics
# match the Python autoRIFT, which is what the pyramid's agreement actually depends on: `colfilt`
# supplies the grid resize, the search-radius widening, the prior average and the hole fill
# (`autoRIFT.py:509-808`), so a border or NaN convention that differs by one moves the answer at
# every level.
#
# These cases are the reference's measured output, generated by
# `tools/python_ref/gen_fixtures.py colfilt bwareaopen` against `autoRIFT.colfilt` itself.
if !has_fixtures()
    @info "Fixture corpus absent; skipping the colfilt comparison."
else
    const COLFILT_FN = Dict("max" => windowmax, "min" => windowmin, "mean" => windowmean,
                            "median" => windowmedian, "range" => windowrange, "mad" => windowmad)

    # `chunk_size = 1` is the comparison that isolates the reducer. At the reference's default of 4
    # its even-kernel output carries a per-chunk seam (below), which is a property of its chunking
    # rather than of the reduction, so mixing the two would report one as the other.
    @testset "colfilt, one chunk" begin
        # `mad` is excluded from exact equality and asserted separately: its two nested medians are
        # accumulated in a different order on each side, which is Float32 rounding rather than a
        # semantic difference.
        for opt in ("max", "min", "mean", "median"), field in ("plain", "holed"), k in (2, 3, 4, 5, 9)
            f = fixture("colfilt/$(opt)_$(field)_k$(k)_c1")
            @test nan_identical(COLFILT_FN[opt](f.arrays.src, k), f.arrays.expected)
        end

        # `range` agrees on every finite value and differs on one case by design: an all-NaN window.
        # `frange` starts from `±inf` and compares with `<`/`>`, which a NaN never satisfies, and
        # option 4 — unlike option 0 — has no `isneginf -> nan` cleanup (`autoRIFT.py:1540-1546`).
        # So the reference returns `-inf` where a window holds nothing and AutoRIFT.jl returns `NaN`.
        #
        # `NaN` is the answer that composes: the value feeds `ceil(Int, ...)` when a decimated cell's
        # search radius is built (`_decimate_level`), where `-inf` and `NaN` both throw, and it is
        # `NaN` that every other reduction here already returns for an empty window. The divergence is
        # also unreachable on production input, where the captured priors carry no `NaN` at all
        # (measured on the golden Landsat case: 0 of 5,475,584 points in `Dx0` or `Dy0`).
        for field in ("plain", "holed"), k in (2, 3, 4, 5, 9)
            f = fixture("colfilt/range_$(field)_k$(k)_c1")
            got = windowrange(f.arrays.src, k)
            exp = f.arrays.expected
            @test all(eachindex(exp, got)) do i
                a, b = exp[i], got[i]
                (isnan(a) && isnan(b)) || (a == -Inf32 && isnan(b)) || a == b
            end
        end

        for field in ("plain", "holed"), k in (2, 3, 4, 5, 9)
            f = fixture("colfilt/mad_$(field)_k$(k)_c1")
            got = windowmad(f.arrays.src, k)
            exp = f.arrays.expected
            # Same validity everywhere, and the values within Float32 accumulation error.
            @test map(isnan, got) == map(isnan, exp)
            @test all(i -> isnan(exp[i]) || abs(exp[i] - got[i]) < 1.0f-5, eachindex(exp, got))
        end
    end

    @testset "colfilt's even-kernel chunk seam is the reference's" begin
        # `colfilt` takes the left margin as `(k - 1) ÷ 2` where `generic_filter` centres its window
        # at `k ÷ 2` (`autoRIFT.py:1490-1499`). For even `k` those differ by one, so the first output
        # column of each chunk after the first is written from a window one column off — `N_chunks - 1`
        # corrupted columns per row, at whatever column boundaries the chunk split happens to fall on.
        #
        # This is asserted rather than reproduced, and the assertion is what makes that a decision:
        # AutoRIFT.jl agrees with the reference **exactly** at every even kernel when the reference
        # runs one chunk, so the disagreement at four chunks is entirely the seam. Only the non-base
        # pyramid levels use even kernels.
        #
        # Where the seams fall is arithmetic, not a property of the data, so it is derived here from
        # `colfilt`'s own chunk split and the derivation is what the columns are checked against. On
        # this 24x30 field with four chunks they are columns 9, 16 and 23 for every reducer and both
        # even kernels. A reducer may still *agree* at a seam by chance — the wrong window can hold
        # the same maximum — so the assertion is that a disagreement occurs nowhere else, not that
        # every seam disagrees.
        seams = let nc = 30, chunks = 4, k = 2
            n = nc - k + 1
            per = fill(n ÷ chunks, chunks)
            for i in 1:(n % chunks)
                per[i] += 1
            end
            starts = cumsum(per) .- per
            (starts[2:end] .+ (k - 1) ÷ 2) .+ 1        # 0-based chunk start to 1-based column
        end
        @test seams == [9, 16, 23]

        for opt in ("max", "min", "mean", "median"), k in (2, 4)
            f1 = fixture("colfilt/$(opt)_plain_k$(k)_c1")
            f4 = fixture("colfilt/$(opt)_plain_k$(k)_c4")
            got = COLFILT_FN[opt](f1.arrays.src, k)
            @test size(f1.arrays.src) == (24, 30)
            # One chunk: exact.
            @test nan_identical(got, f1.arrays.expected)
            # Four: every disagreement is on a seam column, and at least one seam does disagree.
            bad = [j for j in axes(f4.arrays.expected, 2)
                   if any(i -> !nan_same(f4.arrays.expected[i, j], got[i, j]),
                          axes(f4.arrays.expected, 1))]
            @test !isempty(bad)
            @test bad ⊆ seams
        end
    end

    @testset "bwareaopen: eight-connected, size1 exclusive" begin
        # `small_components` returns the components *below* `minsize`; `bwareaopen` keeps those at or
        # above it. Within the mask the two are complements, which is the relation asserted here —
        # so this pins both the connectivity and the exclusive bound in one comparison.
        #
        # The `diagonal` case is the one that discriminates: a 16-long diagonal chain is one
        # component of 16 under eight-connectivity and sixteen of one under four, so it survives every
        # `size1` here in the first reading and none of them in the second.
        for name in ("speckle", "diagonal", "blocks", "empty", "full"), size1 in (2, 5, 9)
            f = fixture("bwareaopen/$(name)_s$(size1)")
            mask = f.arrays.src .!= 0
            kept = f.arrays.expected .!= 0
            small = small_components(mask, size1)
            @test kept == (mask .& .!small)
            # Nothing outside the mask is ever reported, on either side.
            @test !any(small .& .!mask)
            @test !any(kept .& .!mask)
        end
    end
