...
| **Scene rotation derived from pixels, not from the orbit** | `_fft_filter` recovers the along- and cross-track angles from the valid-data region — largest connected component, contour, `minAreaRect`, four quadrant extremes (`autoRIFT.py:153-206`) — where the scene's `_ANG.txt` carries `EPHEMERIS_ECEF_{X,Y,Z}` and gives the ground-track heading directly once transformed into the raster's own CRS. The scene is rotated because the acquisition track does not align with map coordinates, and near the poles none does; the projection step is not optional, since a grid bearing differs from a true one by the meridian convergence — 3.2° here, more at higher latitude. Measured on both scenes of `LT05_L1TP_060018_19851028`: the projected orbit's **cross-track agrees to 0.03° and 0.04°**, while its along-track is a constant **−1.89°** from the reference's. That gap is the reference's `nanmax` over two edge slopes biasing it toward the worse-conditioned edge — and its two axes are correspondingly *not* perpendicular, −20.245° against −18.398°, which a real cross-track direction cannot be. | Once the L4/5 pairs agree. Then read the angles from `_ANG.txt`, projected into the scene CRS, and measure what the derivation was costing. `tools/golden/mtl.jl` keeps the ephemeris reader and `FastGeoProjections`/`Proj` does the transform. |
