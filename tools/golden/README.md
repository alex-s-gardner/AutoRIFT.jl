...
### Why the disagreement grows with chip size

One number explains it. Each level's answer is spread from the nodes it kept to the merged points it
owns, and that leverage rises steeply:

| chip | nodes measured / own grid | kept nodes | merged points owned | **points per kept node** |
|---:|---:|---:|---:|---:|
| 96 | 35.79% | 1,110,129 | 1,215,214 | **1.09** |
| 192 | 23.28% | 243,471 | 341,225 | **1.40** |
| 384 | 9.37% | 22,012 | 87,913 | **3.99** |
| 768 | 7.41% | 3,834 | 118,799 | **30.99** |

At the base level a reported value is very nearly one measurement. At chip 768 the reference reports
118,799 values from 3,834 surviving measurements, so one node's answer becomes about thirty-one
outputs. The level is a propagation stage rather than a measurement stage: at chip 384 its field is
**79% inherited** from the previous level (83,681 points patched from `DxF0`) against **21% its own**
(22,012). A small difference in which nodes survive is therefore multiplied, and the reference's own
`InterpMask` shows the filled fraction rising the same way — 8.65%, 11.98%, 12.46%, **17.70%**.

**The base level and the coarse levels fail for different reasons, and conflating them is a trap.**
Within the base level, disagreement tracks peak strength exactly as `UInt8` tie-breaking predicts: on
the NISAR L2 case the weakest correlation quartile is **31× more likely** to disagree beyond 0.5 px
than the strongest (0.31% against 0.01%). Within chip 768 that relationship is **absent** — the
strongest quartile disagrees *most* (+2.30 px against +2.09) and the rate is flat near 93% across every
quartile. Strong peaks have no ties, so tie-breaking cannot be the coarse-level mechanism.

Two consequences worth keeping:

- **Both element types have to be checked, and for different reasons.** The golden cases run `UInt8`
  because the production driver sets `DataType = 0`, and that is where tie-breaking lives. Production
  will move to `Float32`, where `tools/ab` measures the correlator **bit-identical at every point**
  against 98.60% for the byte path. A gate on one says nothing about the other.
- **`Float32` is unreachable on a NISAR case.** `loadProduct` (`testautoRIFT.py:238-249`) hard-casts to
  `uint8` under `nisar_flag`, so `DataType = 1` never reaches the correlator there and
  `CAPTURE_FLOAT32` cannot be used to separate quantization from anything else. The byte-vs-float floor
  has to be measured on an optical or Sentinel-1 case.
...
| **`UInt8` quantization before correlating** | `uniform_data_type` rescales each image by its own mean and standard deviation and quantizes to 256 levels before the correlator sees it (`autoRIFT.py:359-384`), discarding precision the filtered float field already has. Nothing about the correlator needs it, and it costs accuracy at every point — the reference's own byte path disagrees with its float path. It exists because the production driver sets `DataType = 0`. | Once every case agrees. The reference's two entry points (`arImgDisp_u` vs `arImgDisp_s`) are what make this a matching requirement rather than a choice: `tools/ab/README.md` records the float path bit-identical while the byte path fails on 1.7% of points by up to 36 px. **Production is moving to `Float32`, so both paths need a gate, not one replacing the other** — and the cost is now quantified: within the base level the weakest correlation quartile is 31x more likely to disagree than the strongest, which is the tie-breaking this creates. Not testable on NISAR, where `loadProduct` hard-casts to `uint8`. |
| **Scene rotation derived from pixels, not from the orbit** | `_fft_filter` recovers the along- and cross-track angles from the valid-data region — largest connected component, contour, `minAreaRect`, four quadrant extremes (`autoRIFT.py:153-206`) — where the scene's `_ANG.txt` carries `EPHEMERIS_ECEF_{X,Y,Z}` and gives the ground-track heading directly once transformed into the raster's own CRS. The scene is rotated because the acquisition track does not align with map coordinates, and near the poles none does; the projection step is not optional, since a grid bearing differs from a true one by the meridian convergence — 3.2° here, more at higher latitude. Measured on **all six scenes of the three L4/5 pairs** (`tools/golden/orbit.jl`, gate `5.orbit`): the projected orbit's **cross-track agrees to 0.104° at worst**, while its along-track sits **1.45° to 1.89° below** the reference's — a spread, not the constant one pair alone suggested. The error is the reference's and it is confined to one axis: the orbit's two directions are perpendicular to the last digit by construction, and the reference's are **91.42° to 91.86°** apart, never 90°. Its own non-perpendicularity accounts for the along-track gap case by case, which is what identifies `nanmax` over two edge slopes as the cause — the worse-conditioned edge wins, and only the along-track pair is drawn from two edges that disagree. | Once the L4/5 pairs agree. Then make `Destripe`'s angles come from `_ANG.txt` projected into the scene CRS, and measure what the pixel derivation was costing. `tools/golden/mtl.jl` has the ephemeris reader (`ang_ephemeris`, `orbit_scan_angles`) and `ArchGDAL` does the transform. |
...
| **The Wallis local variance is computed in a numerically poor form** | `_masked_boxstd` is `sqrt(E[x²] − E[x]²)` clipped at zero, matching `_preprocess_filt_std` (`autoRIFT.py:47-56`). Against an exact `Float64` truth on a 256² scene of mean 130 and standard deviation 25 at `width = 5`, that form is off by a median of **0.54** and up to 5.79 where squaring the deviations about the measured mean is off by **1.5e-6** — and the cancellation can go negative, which is why the reference clips it rather than taking the square root of a negative number. **Adopted deliberately**: the stable form moves 90% of a filtered field by a median of 0.113 on a field clamped to ±3, and on `LT05_L1GS_001013` scene 1 it moved the band-reject's decision across a ratio test that clears by 3.2%, so a whole scene's output changed. | After the golden set agrees. `dev/CORRECTNESS.md` item 4 carries it. Judge against an exact `Float64` truth, not against agreement — both implementations use the same form now, so the golden set cannot see the difference. Note a Gaussian random field cannot see it either: the two forms agree to 4.4e-5 there, because the cancellation needs the bright low-contrast regions a real scene has. |

One difference runs the *other* way — AutoRIFT.jl is more nearly correct and deliberately does not
match:
...
| The coarse-level residual is `UInt8` tie-breaking | Within the base level it holds — the weakest correlation quartile is 31x more likely to disagree beyond 0.5 px than the strongest. Within chip 768 it inverts: the *strongest* quartile disagrees most, +2.30 px against +2.09, flat near 93% across every quartile. Strong peaks have no ties | Tie-breaking is a **base-level** mechanism only. It does not explain the coarse levels. |
| The coarse-level residual is peak selection | `peak_ratio` measures whether a rival peak exists, and on the large base-level errors its median is 8.46 against 1.72 for agreeing points — the peak is 8.5x its nearest rival, i.e. unambiguous. `correlation` predicts these errors well (median 0.0332 against 0.0596) and `peak_ratio` ranks them backwards | Not peak selection. Confirms `src/peak.jl:191-208`: gate on `correlation`, not `peak_ratio`. |
| The coarse-level residual is level ownership | `chip_size` compared directly on both sides over a 401² window: ownership agrees on 95.75%, and where it differs the residual has the *same* spread as where it agrees (std 0.2542 against 0.2541). Largest per-pair mean in the confusion matrix is −0.0474 px | Not ownership — but see the next row, which is how this measurement was itself misread. |
| A window's per-level figures are comparable to the reference's | They are not: level assignment is extent-dependent. At node (246,1722) our `dx` is bit-exact at −3.250000 for centred windows of half 8, 32, 100 and 200 — all chip 96 — and −4.561503 in a 401² window at rows 124–524, which assigns it to chip 768. The extent accounts for 23.8% of that window's `ddx` residual mass and 28.6% of its points beyond 0.5 px | **A window is a per-point diagnostic, never a per-level measurement.** `window_endpoint.jl` now withholds the per-level table and reports the level disagreement rate instead. |
| Spatial autocorrelation proves a convention error here | It proved one for `Dy0` and does not generalise unmodified. The whole-window lag-1 is +0.80 — the same as the `Dy0` bug — but that pools a measured level with two where **100%** of points disagree by construction, and a smooth field differenced against itself autocorrelates trivially. A convention scan on the worst quadrant is decisive against: median \|ddx\| is **0.00000** with no correction, against 0.09375 for every whole-pixel roll and ≥0.0625 for every sub-pixel offset | The criterion needs a population of comparable points. Apply it per level, and read the level's on-lattice fraction first. |
| Matching the reference's level decimation makes agreement worse | It does not; the earlier attempt changed one of two coupled quantities. `_level_decimation` is now `chip_size.X ÷ chip_size_min.X` and at level 2 of the L2 GSLC case our level grid is **572x572, the reference's own shape**, where the old `min(sx, sy)` gave 1144x1144 — a different number of points entirely. The reduced prior then agrees on 327,145 of 327,184 nodes (99.988%); 27 of the 39 residuals are the documented 1 px `INTER_NEAREST` mapping difference. The coupled quantity was `_grid_step` (next row): at the wrong stride *and* a zero shift the two errors partly cancelled, which is why moving one alone looked like a regression | **Two bugs, one symptom.** A rung that recomputes the reference's rule instead of calling production's cannot see either — every stride in `stages.jl` derived `chip ÷ chip0`, so the ladder was green on the cases that disagreed. |
| The nodata fill is always zero, so `_grid_step`'s zero guard is enough | The fill survives the half-sample snap as a constant that need not be zero: both NISAR grids arrive filled with `0.5`, carried to `1.5`, covering **55.7%** of the L2 array and **56.8%** of the L1 one. The guard excluded steps *touching a zero*, so it never fired, and 2,895,601 zero steps inside the L2 fill outvoted 2,297,235 real 48 px ones. `_grid_step` returned **0.0**, `_cell_centres` shifted by nothing, and every coarse node sat at its cell's first point — the exact failure the helper exists to prevent | Excluding the zero *step* fixes it, which a constant region produces whatever its value. Now 48/24 on the L2 grid, 33/19 on the rotated L1 one; rung 3.1 reports a 192 px cell where it printed `NaN of a 0 px cell`. |
| The coupled difference is the reducer's even-window origin | Measured against scipy directly: `generic_filter` on an even window extends `w//2` before the origin and `w-1-w//2` after — 1/0 at w=2, 2/1 at w=4, 4/3 at w=8. `AutoRIFT._window_margins` is `lx = w ÷ 2`, `right = w-1-lx`, identical at all three | Not the reducer alignment. |
| The coupled difference is coarse node placement, or a 0-based/1-based slip | The reference's `INTER_AREA` block centre for output `k`, converted to 1-based fine indices, is `(k-1)·d + (d-1)/2 + 1`. `_decimate_level` takes `rows = 1:stride:nr` and `_cell_centres` shifts by `(stride-1)/2`, giving `(k-1)·d + 1 + (d-1)/2`. Identical at d = 2, 4, 8 | Not the placement rule — but it is a *function of the stride*, so a wrong stride moves every node. That is what made the two bugs above look like one unfixable one. |
