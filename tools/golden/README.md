...
| **Level decimation derived from the X axis alone** | `Scale = ChipSizeUniX[i] / self.ChipSize0X` (`autoRIFT.py:820`, reciprocal at `:510`) reads only the x extent, so a level's grid is coarsened by the same factor in both axes however anisotropic the chip is. On NISAR's 96x52 chip the reference's own traced grids go 2328x2304 -> 1164x1152 -> 582x576 -> 291x288, i.e. it decimates y by 2, 4 and 8 for a chip that is 52 px tall against a 48 px grid spacing — a coarse grid several times coarser than the chip supports, so a level posts estimates from a footprint its own geometry cannot corroborate. AutoRIFT.jl's `_level_decimation` takes `min(sx, sy)` and so returns 1, 2, 4 instead. **This is a real divergence and it is not yet resolved:** matching the x-only rule in isolation raises level agreement from 95.66% to 98.12% and makes the chip-192 lattice match, but *triples* the rate at which our field disagrees with its own neighbours relative to the reference's — 3.0x to 10.2x — and lowers `exact`. So a second, coupled difference compensates for it on the reference's side. | Blocked on finding that second difference. The measurements to reproduce: per-level lattice occupancy (ours 98.12% on 1/64 at chip 192 against the reference's 0.00%), and neighbour-disagreement rate per axis (our `dx` 3.0x the reference's, `dy` 1.0x). |
| **Rectangular chips are not obviously handled correctly by either implementation** | The two disagree about the decimation above, and neither rule is self-evidently right for a 96x52 chip. An x-derived stride undersamples y; a `min`-derived stride posts more estimates per chip footprint in x than the reference does. The physically defensible rule is probably per-axis, which is what neither does. Recorded so the question is not lost inside the agreement work. | After the golden set agrees. Then measure a per-axis decimation against both, on an anisotropic case, and judge on coverage and on neighbour consistency rather than on agreement. |
