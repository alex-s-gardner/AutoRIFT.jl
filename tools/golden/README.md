...
| **Scene rotation derived from pixels, not from the orbit** | `_fft_filter` recovers the along- and cross-track angles from the valid-data region — largest connected component, contour, `minAreaRect`, four quadrant extremes (`autoRIFT.py:153-206`) — where the scene's `_ANG.txt` carries `EPHEMERIS_ECEF_{X,Y,Z}` and gives the true ground-track heading directly. Measured on `LT05_L1TP_060018_19851028`: the ephemeris gives 71.63° and the reference logs 71.60°, 0.03° apart. On its `L1GS`/T2 partner from the *same descending pass* — orbit azimuth 198.35° against 198.37° — the reference logs 75.49° against the ephemeris's 71.65°, off by 3.84°, because a pixel-derived estimate follows how the valid region happens to be clipped. Its cross-track values are also not perpendicular to its along-track ones, off by 1.85° and 2.03°, which a real cross-track direction cannot be. | Once the L4/5 pairs agree. Then read the angles from `_ANG.txt` and measure what the derivation was costing. `tools/golden/mtl.jl` keeps the ephemeris reader so this is one call away. |
