...
Any argument omitted keeps its current value, so `reinit!(cache; secondary = next)` advances
one image of a time series while holding the other fixed.
...
function reinit!(cache::Cache; reference = nothing, secondary = nothing,
    ...
    ref = isnothing(reference) ? cache.raw.reference : reference
    sec = isnothing(secondary) ? cache.raw.secondary : secondary
    rv = isnothing(reference_valid) ? cache.raw.reference_valid : reference_valid
    sv = isnothing(secondary_valid) ? cache.raw.secondary_valid : secondary_valid
    size(ref) == size(cache.raw) && size(sec) == size(cache.raw) || throw(DimensionMismatch(
        "new images are $(size(ref)) and $(size(sec)) but the cache was built for " *
        "$(size(cache.raw)); the grid and output arrays are sized to that. Build a new " *
        "cache with `init` for a different image size."))

    cache.raw = ImagePair(ref, sec; reference_valid = rv, secondary_valid = sv)
    cache.prepared = _prepare(cache.raw, cache.params)
...
_prepare(pair::ImagePair, p::Params) = quantize(preprocess(pair, p.preprocess), p.quantize)
